...
/// 从 super-dashboard 旧数据迁移
/// 读取 ~/Library/Application Support/DevDash/userData/vault.enc
/// 使用相同的 Keychain 服务名，密钥自动兼容
#[tauri::command]
pub fn vault_import_from_super_dashboard(state: State<'_, VaultState>) -> VaultSaveResult {
    // 必须先解锁
    let key_guard = state.master_key.lock().unwrap();
    let key = match key_guard.as_ref() {
        Some(k) => *k,
        None => {
            return VaultSaveResult {
                success: false,
                error: Some("请先解锁保险箱再迁移数据".to_string()),
            }
        }
    };
    drop(key_guard);
    ...
    let old_path = dirs::home_dir()
        ...
        .join("Library/Application Support/DevDash/userData/vault.enc");
    ...
    match load_vault(
        ...
        Ok(store) => {
            match save_vault(&state.data_path, &key, &store) {
                Ok(_) => VaultSaveResult {
                    success: true,
                    error: None,
                },
